...
1. Paper/Lab report  
   论文或实验报告的呈现形式均为在BB上交文档，在DDL前一周的课上一般会讲解作业要求，迟交会有相应扣分。BB上有链接可查看分数与评价（一般实际出分时间都比原本承诺时间晚一周左右），不同教授的打分标准略有不同，对分数有异议可在规定时间内联系课程organizer。
2. Presentation  
   汇报方式为借助海报（poster）或PPT进行英文脱稿汇报，评分方式为海报分数+pre分数，或由教授和同学共同对PPT汇报打分。部分Presentation为小组合作作业。
3. 关于BMI专业，有网页搭建和代码提交等形式，比较考验自学能力。

#### Final 期末考试

大一均为单项选择题，大二开始加入简答题，考试内容大部分出自平日上课的PPT。不及格（一般为平均分的40%，会根据难度调整）须在假期集中补考。补考有邮件通知，需关注。
...
2. assignments 作业，请注意所有的作业都有一定的分值，所以请避免做完作业不提交的情况（血的教训）。
   有时教授会布置有额外分数的bonus assignment，不做不会扣分，但做了可以加分。
